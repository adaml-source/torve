...
from app.billing import ENTITLEMENT_LIFETIME, ENTITLEMENT_SUBSCRIPTION, grant_entitlement
...
def redeem_code(
    ...
    if prior_same:
        ...
        return RedemptionResult(True, "access_already_granted",
    ...
    # Check existing lifetime (safe noop without consuming code)
    ...
    if has_lifetime:
        _record_attempt(db, code_row.id, user_id, source_ip, user_agent, "access_already_granted")
        db.commit()
        _log.info("REBATE_CODE_REDEEM_NOOP user=%s code_prefix=%s reason=already_has_lifetime",
                   user_id, code_row.code_prefix)
        return RedemptionResult(True, "access_already_granted",
                                "You already have lifetime access.")

    # Atomic redemption. Grant type follows the code's grant_duration_days:
    # NULL → lifetime (existing behaviour for all pre-2026-04-27 codes);
    # positive int N → subscription_monthly that expires in N days from
    # redemption time, no auto-renew. The success message follows suit so
    ...
    try:
        ...
        source_ref = f"rebate:{code_row.id}"
        if code_row.grant_duration_days and code_row.grant_duration_days > 0:
            expires_at = datetime.now(timezone.utc) + timedelta(days=code_row.grant_duration_days)
            grant_entitlement(
                db, user_id, SOURCE_REBATE, source_ref,
                ENTITLEMENT_SUBSCRIPTION,
                product_id=f"rebate:{code_row.code_prefix}",
                expires_at=expires_at,
                auto_renew=False,
            )
            success_msg = f"Premium granted for {code_row.grant_duration_days} day{'s' if code_row.grant_duration_days != 1 else ''}."
        else:
            grant_entitlement(
                db, user_id, SOURCE_REBATE, source_ref,
                ENTITLEMENT_LIFETIME, product_id=f"rebate:{code_row.code_prefix}",
            )
            success_msg = "Lifetime access granted."
        db.commit()

        _log.info("REBATE_CODE_REDEEM_SUCCESS user=%s code_prefix=%s campaign=%s duration=%s",
                   user_id, code_row.code_prefix, code_row.campaign_name,
                   code_row.grant_duration_days or "lifetime")
        return RedemptionResult(True, None, success_msg)
