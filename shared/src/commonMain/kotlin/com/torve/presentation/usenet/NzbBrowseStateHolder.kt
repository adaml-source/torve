 ...
 * (Sports, Adult on desktop, future mobile/TV equivalents).
 *
 * The composables themselves use `remember`, which is scoped to the
 * Compose call site — navigating away (e.g. when the user taps Play
 * and the playback overlay swaps in) tears down the call site and
 * discards the search query and result list. Storing it here, in
 * plain process memory, lets the page rehydrate the same query +
 * results + scroll offset on return without round-tripping the
 * indexer.
 *
 * Cleared automatically on app restart (singleton in classloader
 * memory).
 */
