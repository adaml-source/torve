...
Per-release procedure once `https://api.torve.app/releases/appcast.xml`
is live (PR #1 merged + deployed).

## One-time setup

1. Get the backend's admin secret out of the prod `.env` and put it
   in the local environment as `TORVE_ADMIN_SECRET`. This matches the
   backend's `PADDLE_ADMIN_SECRET`. Examples:

   ```powershell
   # PowerShell (this session only)
   $env:TORVE_ADMIN_SECRET = '<paste-the-secret>'

   # Or persist it for the user
   [Environment]::SetEnvironmentVariable('TORVE_ADMIN_SECRET', '<paste>', 'User')
   ```
...
After it returns success, end users on a prior version pick up the
...
- **The handoff verifies SHA-256 before launching the MSI.** If
  ...
  install.
- **Admin secret rotation**: if you rotate `PADDLE_ADMIN_SECRET` on
  the backend, also rotate `TORVE_ADMIN_SECRET` locally. The two are
  the same value by design (the admin endpoint reuses the existing
  Paddle admin auth pattern).
