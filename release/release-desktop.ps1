...
#   * Admin secret in $env:TORVE_ADMIN_SECRET (matches the backend's
#     PADDLE_ADMIN_SECRET; the admin endpoint takes it via X-Admin-Secret
#     header).
...
if (-not $adminSecret) {
    throw "TORVE_ADMIN_SECRET env var is not set. Set it to the backend's PADDLE_ADMIN_SECRET value before running."
}
