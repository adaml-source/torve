"""Tests for Google Play and Amazon purchase verification hardening."""
...
def test_gp_exact_product_mismatch_rejected(client, db, monkeypatch):
    """Wrong Google Play product ID is rejected even in non-production."""
    ...
    assert r.json()["message"] == "Product not recognized."


def test_gp_exact_product_match_proceeds(client, db, monkeypatch):
    """Correct product ID passes validation (still fails at Google API if not configured)."""
    ...
    assert r.status_code == 200
    # Passes product check but fails at Google API (not configured)
    assert r.json()["verified"] is False
    assert "not configured" in r.json()["message"].lower() or "could not be verified" in r.json()["message"].lower()


def test_gp_missing_product_id_production_fails_closed(client, db, monkeypatch):
    """In production mode, missing GOOGLE_PLAY_PRODUCT_ID rejects all."""
