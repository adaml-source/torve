...
fun V2App(
    ...
    fun premiumGuard(reason: String, action: () -> Unit) {
        if (hasPremium) action() else premiumGateReason = reason
    }
    ...
    fun prepareSourcePicker(
        ...
        if (!hasPremium) {
            premiumGateReason = if (forDownload)
                "Downloads are a Premium feature."
            else
                "Browsing sources for playback is a Premium feature. Trailers stay free."
            return
        }
