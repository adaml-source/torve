...
def register_device(
    ...
    # Device cap only applies to users with real, live premium access.
    from app.billing import check_premium_active
    if check_premium_active(db, uid):
        locked_ids = (
            db.execute(
                select(Device.id)
                .where(Device.user_id == uid, Device.is_active == True)  # noqa: E712
                .with_for_update()
            )
            .all()
        )
        ...
        effective_cap = (user_row.device_cap_override if user_row and user_row.device_cap_override
                         else settings.MAX_DEVICES_PER_ACCOUNT)

        if active_count >= effective_cap:
            _log.warning("DEVICE_LIMIT_HIT user=%s active=%d max=%d platform=%s",
                          uid, active_count, effective_cap, body.platform)
            active_devices = _active_devices_query(db, uid).all()
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=DeviceLimitError(
                    message=f"You have reached your {effective_cap}-device limit.",
                    active_devices=active_devices,
                    max_devices=effective_cap,
    ...
    db.commit()
    db.refresh(device)
    _log.info("DEVICE_REGISTERED user=%s device=%s type=%s platform=%s stable_id=%s",
              uid, device.id, device.device_type, device.platform, device.stable_device_id)
    return device
