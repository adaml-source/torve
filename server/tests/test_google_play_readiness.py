...
def test_verify_returns_config_missing_when_not_ready(_gp_env, client, test_user):
    # Product ID configured so product validation passes; SA JSON and
    # package name still missing → inner readiness gate in
    # _verify_google_play_token fails closed with error_code=config_missing.
    settings.GOOGLE_PLAY_LIFETIME_PRODUCT_ID = "com.torve.pro.lifetime"
    r = client.post(
    ...
    # Generic user-safe message; no internal detail leaked.
    assert "could not be verified" in body["message"].lower()
