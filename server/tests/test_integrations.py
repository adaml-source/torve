...
def test_device_only_credentials_returns_empty(client, test_user, auth_header):
    ...
    r = client.get("/me/integrations/local_addon/credentials", headers=auth_header)
    assert r.status_code == 200
    assert r.json()["storage_mode"] == "device_only"
    assert r.json()["credentials"] == {}


def test_account_mode_credentials_returns_decrypted(client, test_user, auth_header):
...
def test_device_only_ignores_sent_credentials(client, test_user, auth_header):
    ...
    r = client.get("/me/integrations/test_addon/credentials", headers=auth_header)
    assert r.json()["credentials"] == {}
...
def test_switch_mode_account_to_device_only(client, test_user, auth_header):
    ...
    r3 = client.get("/me/integrations/switchable/credentials", headers=auth_header)
...
def test_user_isolation(client, test_user, test_user_b, auth_header, auth_header_b):
    ...
    r2 = client.get("/me/integrations/real_debrid/credentials", headers=auth_header_b)
    assert r2.status_code == 404


def test_integration_survives_reauth(client, test_user, db):
    ...
    r2 = client.get("/me/integrations/trakt/credentials", headers=headers2)
...
def test_patch_merges_new_keys_preserves_existing(client, test_user, auth_header):
...
def test_patch_overwrites_matching_key(client, test_user, auth_header):
    client.put("/me/integrations/trakt", headers=auth_header, json={
        "integration_type": "trakt",
...
def test_patch_empty_credentials_is_noop(client, test_user, auth_header):
