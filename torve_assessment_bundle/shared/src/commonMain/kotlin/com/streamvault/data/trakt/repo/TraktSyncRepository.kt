package com.streamvault.data.trakt.repo
...
import com.streamvault.data.trakt.TraktRatingMovie
import com.streamvault.data.trakt.TraktRatingShow
...
import com.streamvault.data.trakt.db.mapRatingResponseToCache
...
class TraktSyncRepositoryImpl(
    ...
    override suspend fun syncRatingsFromTrakt() {
        ...
        database.streamVaultQueries.clearTraktRatings()

        val now = Clock.System.now().toEpochMilliseconds()
        ratings.forEach { item ->
            val cached = mapRatingResponseToCache(item, now) ?: return@forEach
            database.streamVaultQueries.upsertTraktRating(
        ...
        database.streamVaultQueries.upsertTraktSyncState(
    ...
    override suspend fun getUserRating(tmdbId: Int, mediaType: MediaType): Int? {
        val row = database.streamVaultQueries.getTraktRatingByTmdb(
    ...
    override suspend fun setUserRating(tmdbId: Int, mediaType: MediaType, imdbId: String?, rating: Int?) {
        ...
        if (normalized == null) {
            database.streamVaultQueries.removeTraktRating(key)
        } else {
            val now = Clock.System.now().toEpochMilliseconds()
            database.streamVaultQueries.upsertTraktRating(
                media_key = key,
    ...
    override suspend fun flushPendingWrites(maxItems: Int): Int {
        ...
        val now = Clock.System.now().toEpochMilliseconds()
        val items = database.streamVaultQueries.getPendingTraktQueue(now, maxItems.toLong()).executeAsList()
        var successCount = 0
        for (item in items) {
            ...
            if (payload == null) {
                database.streamVaultQueries.deleteTraktQueueItem(item.id)
                continue
            }
            ...
            try {
                ...
                database.streamVaultQueries.updateTraktQueueAttempt(
    ...
    override suspend fun clearLocalData() {
        database.streamVaultQueries.clearTraktRatings()
        database.streamVaultQueries.clearTraktSyncState()
        database.streamVaultQueries.clearTraktQueue()
    }

    private suspend fun enqueue(actionType: String, payload: TraktQueuePayload) {
        val now = Clock.System.now().toEpochMilliseconds()
        val id = "trakt_q_${now}_${Random.nextInt(1000, 9999)}"
        database.streamVaultQueries.insertTraktQueueItem(
            id = id,
