"""Tests for /me/purchases/restore + legacy /purchases/restore.

Background: live mobile build 1.0.38 POSTs to /purchases/restore on
"Restore Purchase" tap. Pre-fix the backend had no such route — every
tap returned 404 and the user saw "Google Play restore could not be
completed". The new endpoint refreshes premium booleans from the
canonical entitlement table and pulls back any lifetime grant for the
user's email from the ledger.
"""
...
def test_restore_recomputes_stale_premium_flag(client, db):
    """User has an active lifetime entitlement but the cached
    has_lifetime_access boolean is stale — restore should fix it.
    Mirrors the real-world scenario where someone manually edits
    User flags or where a prior recompute path missed a write."""
    user = _make_user(db)
    ...
    assert body["has_lifetime_access"] is True
...
def test_restore_pulls_lifetime_grant_from_ledger(client, db):
    """User has no entitlement row but a lifetime-grant ledger row matches
    their email — restore should re-provision the entitlement.
    Covers the delete-then-resignup path."""
    user = _make_user(db)
    ...
    # The ledger restore should have created an active lifetime entitlement.
...
def test_restore_no_entitlements_returns_clean_state(client, db):
    """Free user, no entitlements, no ledger row — restore is a no-op
    that returns has_premium_access=False without erroring."""
    user = _make_user(db)
    r = client.post("/me/purchases/restore", headers=_auth(user.id))
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_restore_does_not_snowball_ledger_or_entitlements(client, db):
    ...
    assert active_lifetime_count == 1, "restore should be idempotent"
