...
class SetupWizardCoordinatorTest {
    ...
    @Test
    fun `validating one intent leaves the others untouched (debrid-only path)`() = runTest {
    ...
    @Test
    fun `invalid validator result is persisted as INVALID, never READY`() = runTest {
        val coord = newCoordinator(
            validators = mapOf(
    ...
    @Test
    fun `validator that throws is wrapped as INVALID, never crashes the coordinator`() = runTest {
        val coord = newCoordinator(
            validators = mapOf(
