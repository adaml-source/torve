...
def test_successful_redemption(client, db, monkeypatch):
    ...
    db.refresh(user)
    assert user.has_lifetime_access is True
...
def test_already_has_lifetime_returns_noop(client, db, monkeypatch):
    monkeypatch.setattr("app.config.settings.REBATE_CODE_HMAC_SECRET", "test")
    ...
    r = client.post("/me/rebate-codes/redeem", json={"code": raw},
                     headers={"Authorization": f"Bearer {create_access_token(str(user.id))}"})
    assert r.json()["success"] is True
    assert r.json()["error_code"] == "access_already_granted"

    db.refresh(code_row)
    assert code_row.redeemed_count == 0
...
def test_retry_after_success_returns_stable_response(client, db, monkeypatch):
    """Post-commit retry: same user, same code should return access_already_granted."""
    monkeypatch.setattr("app.config.settings.REBATE_CODE_HMAC_SECRET", "test")
    user = _make_user(db)
    _cleanup_all_attempts(db, user.id)
    raw, code_row = _make_code(db)
    ...
    assert r2.json()["error_code"] == "access_already_granted"
...
def test_access_state_reflects_rebate_entitlement(client, db, monkeypatch):
    monkeypatch.setattr("app.config.settings.REBATE_CODE_HMAC_SECRET", "test")
    user = _make_user(db)
    _cleanup_all_attempts(db, user.id)
    raw, code_row = _make_code(db)
    ...
    assert r.json()["access_tier"] == "premium_lifetime"
    assert r.json()["source"] == "rebate_code"
...
def test_rebate_code_with_duration_grants_subscription(client, db, monkeypatch):
    """A code with grant_duration_days=30 grants a 30-day subscription
    (not lifetime). The cached has_lifetime_access must NOT flip to true."""
    monkeypatch.setattr("app.config.settings.REBATE_CODE_HMAC_SECRET", "test")
    user = _make_user(db)
    _cleanup_all_attempts(db, user.id)
    ...
    assert "30 day" in body["message"]

    db.refresh(user)
    assert user.has_lifetime_access is False, "trial code must not grant lifetime"
    assert user.has_premium_access is True, "trial code grants premium"

    ent = db.query(UserEntitlement).filter(UserEntitlement.user_id == user.id).first()
    assert ent is not None
    assert ent.entitlement_type == "subscription_monthly"
...
def test_rebate_code_without_duration_still_grants_lifetime(client, db, monkeypatch):
    """Pre-existing behaviour: codes with no duration grant lifetime."""
    monkeypatch.setattr("app.config.settings.REBATE_CODE_HMAC_SECRET", "test")
    user = _make_user(db)
    _cleanup_all_attempts(db, user.id)
    ...
    assert r.status_code == 200
    assert r.json()["success"] is True
    assert "Lifetime" in r.json()["message"]
