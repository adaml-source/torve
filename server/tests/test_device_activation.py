...
- Premium users DO get device cap enforcement
- Idempotent reactivation by installation_id
- Revoked devices are not counted as active
- Device-cap responses use structured codes and safe messages
- /me/access-state reports device-cap state correctly
"""
import uuid
...
class TestPremiumDeviceCap:
    ...
    def test_premium_lifetime_cap(self, client, db):
        """Premium lifetime user is capped at MAX_DEVICES."""
        ...
        try:
            ...
            r = client.post("/me/devices/register", headers=_auth(user), json={
                "device_type": "phone",
                "platform": "android",
                "installation_id": f"lt-cap-6-{user.id}",
            })
    ...
    def test_premium_subscription_cap(self, client, db):
        """Premium subscription user is capped at MAX_DEVICES."""
        ...
        try:
            ...
            r = client.post("/me/devices/register", headers=_auth(user), json={
                "device_type": "phone",
                "platform": "android",
                "installation_id": f"sub-cap-6-{user.id}",
            })
            assert r.status_code == 409
        finally:
            _cleanup(db, user)
...
class TestStructuredErrors:
    ...
    def test_device_cap_response_structured(self, client, db):
        ...
        try:
            ...
            r = client.post("/me/devices/register", headers=_auth(user), json={
                "device_type": "phone",
                "platform": "android",
                "installation_id": f"err-6-{user.id}",
            })
            assert r.status_code == 409
            body = r.json()["detail"]
            # Structured fields
            ...
            assert isinstance(body["active_devices"], list)
            assert body["max_devices"] == 5
        finally:
            _cleanup(db, user)

    def test_premium_required_response_structured(self, client, db):
        """Premium-required responses use structured codes."""
        user = _make_user(db)
        db.commit()
        try:
    ...
    def test_access_state_device_cap_for_premium(self, client, db):
        """/me/access-state reports device_limit_reached for premium user at cap."""
        user = _make_user(db, lifetime=True)
        ent = _grant_lifetime(db, user)
        try:
            inst_ids = []
            ...
            assert body["device_block_reason"] == "device_limit_reached"
        finally:
            _cleanup(db, user)
