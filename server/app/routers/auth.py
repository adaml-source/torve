...
def _register_device_on_auth(
    ...
    Enforces the per-account device limit atomically via SELECT FOR UPDATE.
    ...
    if check_premium_active(db, user_id):
        ...
        if active_count >= effective_cap:
            _log.warning("DEVICE_LIMIT_HIT user=%s active=%d max=%d platform=%s (during auth)",
                          user_id, active_count, effective_cap, platform)
            active_devices = (
                db.query(Device)
