...
- Premium users (lifetime and subscription) resolve correctly
...
class TestMeEndpoint:
    ...
    def test_lifetime_user_me(self, client, db):
        ...
        try:
            ...
            assert body["access_tier"] == "premium_lifetime"
            assert body["entitlement_source"] == "admin_grant"
            assert body["entitlement_expires_at"] is None
            assert body["auto_renew"] is None
            # Legacy compat consistent
    ...
    def test_subscription_user_me(self, client, db):
        ...
        try:
            ...
            body = r.json()
            assert body["has_premium_access"] is True
            assert body["access_tier"] == "premium_subscription"
            assert body["entitlement_source"] == "paddle_web"
            assert body["entitlement_expires_at"] is not None
            assert body["auto_renew"] is True
            # Subscription users do not have lifetime
            assert body["has_lifetime_access"] is False
        finally:
            db.delete(ent)
            db.delete(user)
    ...
    def test_no_entitlement_resolves_false(self, client, db):
        """User with stale boolean but no actual entitlement resolves as free."""
        user = _make_user(db, premium=True)  # has_premium_access=True but no entitlement
        try:
            r = client.get("/me", headers=_auth(user))
            assert r.status_code == 200
            body = r.json()
            # resolve_access_state queries entitlements, not cached booleans
            assert body["access_tier"] == "free"
            assert body["has_premium_access"] is False
        finally:
            db.delete(user)
...
class TestPremiumGating:
    """Premium-only features still reject free users."""

    def test_free_user_cannot_generate_pairing_code(
    ...
    def test_free_user_cannot_claim_pairing_code(self, client, free_user, free_auth_header):
    ...
    def test_free_user_cannot_save_integration(self, client, free_user, free_auth_header):
    ...
    def test_free_user_cannot_save_playlist(self, client, free_user, free_auth_header):
...
class TestLegacyFieldConsistency:
    ...
    def test_subscription_user_legacy_fields(self, client, db):
        ...
        try:
            ...
            assert body["has_premium_access"] is True
            assert body["has_lifetime_access"] is False
            assert body["entitlement_source"] == "google_play"
