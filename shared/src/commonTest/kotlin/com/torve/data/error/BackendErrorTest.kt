...
class BackendErrorTest {
    ...
    fun structuredDetailNeverProducesObjectObjectString() {
        ...
        assert("[object Object]" !in message) { "Raw object stringification leaked: $message" }
        assert("{" !in message) { "Raw JSON leaked into message: $message" }
    }
    ...
    fun plainStringDetailNeverContainsJsonBraces() {
        ...
        val error = parseBackendError(body)
        val message = error.message ?: ""
        assert("{" !in message) { "Unexpected JSON in message: $message" }
    }
}
