...
class ParsedStreamRuntimeFilter(
    ...
    suspend fun apply(streams: List<ParsedStream>): ParsedStreamRuntimeFilterResult {
        ...
        val customRulesEnabled = subscriptionRepository
            ?.let { runCatching { it.hasLocallyVerifiedPremiumAccess() }.getOrDefault(false) }
            ?: false
        val rules = if (customRulesEnabled) loadRules() else RuntimeStreamRules()
