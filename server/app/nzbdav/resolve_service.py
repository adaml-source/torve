    ...
    async def resolve(
        ...
        *,
        ...
        result = await self._resolve_inner(
            db, user_id=user_id, content_id=content_id, candidate=candidate
        )
    ...
    async def _resolve_inner(
        ...
        *,
        ...
        if cached is not None:
            ...
            return _build_ready_result(user_id=user_id, cached=cached)
...
def _build_ready_result(*, user_id: uuid.UUID, cached: dict) -> ResolveResult:
    stream_id = cached.get("stream_id") or str(uuid.uuid4())
    token = mint_handoff(user_id=str(user_id), stream_id=stream_id)
