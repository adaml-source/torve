"""
Tests for Pattern B: device-aware verification gate on premium access.
...
Behavior pinned:
  ...
  - require_premium honors the X-Torve-Installation-Id header.
...
# ── core check_premium_active behavior ───────────────────────────────
...
def test_unverified_user_only_originating_device(client, test_user, db):
    try:
        ...
        assert check_premium_active(db, test_user.id, requesting_device_id=d_origin.id)
        # Pattern B kicks in: other device denied while unverified.
        assert not check_premium_active(db, test_user.id, requesting_device_id=d_other.id)
        # Legacy no-device-id path (used by internal jobs / device-cap
        # enforcement) still says "true" — gating is opt-in via the kwarg.
        assert check_premium_active(db, test_user.id)
    finally:
        _cleanup(db, test_user.id)
...
def test_subscription_unverified_originating_only(client, test_user, db):
    try:
        _reset_user_state(db, test_user); _set_verified(db, test_user, False)
        d_origin = _mk_device(db, test_user, f"orig-{uuid.uuid4().hex[:8]}")
        d_other = _mk_device(db, test_user, f"other-{uuid.uuid4().hex[:8]}")
...
# ── /me/access-state surfaces needs_verification ─────────────────────


def test_access_state_needs_verification_true_on_other_device(client, test_user, db):
    try:
        _reset_user_state(db, test_user); _set_verified(db, test_user, False)
        d_origin = _mk_device(db, test_user, f"orig-{uuid.uuid4().hex[:8]}")
        d_other = _mk_device(db, test_user, f"other-{uuid.uuid4().hex[:8]}")
        ...
        # Calling from other device — premium hidden, nag shown.
        state_other = resolve_access_state(
            db, test_user.id, installation_id=d_other.installation_id,
        )
        assert state_other["has_premium_access"] is False
        assert state_other["needs_verification"] is True
    finally:
        _cleanup(db, test_user.id)
...
# ── require_premium honors the header / query param ─────────────────


def test_require_premium_header_routes_to_originating_device(client, test_user, db):
    """Smoke against a real premium-gated endpoint. POST /me/acceleration/outcome
    requires premium. Calling with the originating device's installation_id
    in the header succeeds; calling with another device's id 403s."""
    try:
        _reset_user_state(db, test_user); _set_verified(db, test_user, False)
        d_origin = _mk_device(db, test_user, f"orig-{uuid.uuid4().hex[:8]}")
        d_other = _mk_device(db, test_user, f"other-{uuid.uuid4().hex[:8]}")
        ...
        assert r_other.json()["detail"]["code"] == "premium_required"
    finally:
        _cleanup(db, test_user.id)
