"""Tests for Paddle webhook, entitlements, purchase intents, and refunds."""
...
def test_purchase_creates_entitlement(client, db, monkeypatch):
    ...
    assert r.json()["entitlement_granted"] is True
...
def test_duplicate_idempotent(client, db, monkeypatch):
    ...
    assert r1.json()["entitlement_granted"] is True
...
def test_refund_revokes(client, db, monkeypatch):
    monkeypatch.setattr("app.config.settings.PADDLE_WEBHOOK_SECRET", "")
    monkeypatch.setattr("app.config.settings.PADDLE_PRODUCT_ID", "pro_test")
    monkeypatch.setattr("app.config.settings.PADDLE_PRICE_ID", "pri_test")
    ...
    assert r.json()["has_lifetime_access"] is True

    # Refund
    client.post("/webhooks/paddle", json=_refund_payload(txn))

    # Verify revocation via API
...
def test_second_entitlement_survives_refund(client, db, monkeypatch):
    ...
    db.expire_all()
    db.refresh(user)
    assert user.has_lifetime_access is True  # Second entitlement still active
...
def test_purchase_intent(client, db, monkeypatch):
    ...
    assert r2.json()["entitlement_granted"] is True
...
def test_free_user_blocked_from_integrations(client, db):
    """User without lifetime access cannot save integrations."""
    user = _make_user(db)
    db.commit()
    assert user.has_lifetime_access is False
    ...
    assert "premium_required" in str(r.json())


def test_free_user_blocked_from_playlists(client, db):
    """User without lifetime access cannot save playlists."""
    user = _make_user(db)
    db.commit()
...
def test_revoked_user_loses_premium_access(client, db, monkeypatch):
    """After refund, user can no longer use premium endpoints."""
    monkeypatch.setattr("app.config.settings.PADDLE_WEBHOOK_SECRET", "")
    monkeypatch.setattr("app.config.settings.PADDLE_PRODUCT_ID", "pro_test")
    monkeypatch.setattr("app.config.settings.PADDLE_PRICE_ID", "pri_test")
