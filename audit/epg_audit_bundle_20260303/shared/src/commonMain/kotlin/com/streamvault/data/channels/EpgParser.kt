...
class EpgParser {
    ...
    private val channelPattern = """<channel\s+id="([^"]*)">(.*?)</channel>""".toRegex(RegexOption.DOT_MATCHES_ALL)
    private val displayNamePattern = """<display-name[^>]*>([^<]*)</display-name>""".toRegex()
    private val iconPattern = """<icon\s+src="([^"]*)"[^/]*/?>""".toRegex()
    private val progPattern = """<programme\s+([^>]*)>(.*?)</programme>""".toRegex(RegexOption.DOT_MATCHES_ALL)
    ...
    private fun processStreamingBuffer(
        ...
        while (true) {
            ...
            if (nextBlockStart > 0) {
                buffer.delete(0, nextBlockStart)
            }
    ...
    private fun trimStreamingBuffer(buffer: StringBuilder) {
        if (buffer.length > MAX_STREAM_BUFFER_CHARS) {
            buffer.delete(0, buffer.length - STREAM_BUFFER_RETAIN_CHARS)
        }
    }
    ...
    private fun parseChannelBlock(
        ...
        val displayName = displayNamePattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { id } ?: id
        val icon = iconPattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { null }
    ...
    private fun parseProgrammeBlock(
        ...
        val title = titlePattern.find(body)?.groupValues?.getOrNull(1)?.trim().orEmpty().ifEmpty { "Unknown" }
        val subTitle = if (compactMode) {
            null
        } else {
            subTitlePattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { null }
        }
        val desc = if (compactMode) {
            descPattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.take(320)?.ifEmpty { null }
        } else {
            descPattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.take(800)?.ifEmpty { null }
        }
        val category = if (compactMode) {
            null
        } else {
            categoryPattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { null }
        }
        val icon = if (compactMode) {
            null
        } else {
            iconPattern.find(body)?.groupValues?.getOrNull(1)?.trim()?.ifEmpty { null }
        }
