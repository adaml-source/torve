...
def get_me(
    ...
    """Return the authenticated user's profile with inline canonical access state.

    Clients should use access_tier and has_premium_access for premium gating.
    ...
    _log.info(
        "ME_RESOLVE user=%s access_tier=%s has_premium=%s source=%s",
...
def get_access_state(
    ...
    """Return the authenticated user's current premium access state.
