...
to (user_id, stream_id, exp) and the backend resolves the upstream URL
...
- TTL 5 minutes, scoped to (user_id, stream_id)
...
def mint(
    *, user_id: str, stream_id: str, ttl_seconds: int = TOKEN_TTL_SECONDS
) -> str:
    """Mint a new handoff token. Returns the URL-safe token string."""
    exp = int(time.time()) + int(ttl_seconds)
    payload = {"uid": str(user_id), "sid": str(stream_id), "exp": exp}
...
def verify(token: str) -> HandoffClaims:
    ...
    return HandoffClaims(user_id=uid, stream_id=sid, exp=exp)
