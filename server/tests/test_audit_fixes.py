...
def test_expired_subscription_loses_premium_on_require(client, db):
    """A user with only an expired subscription should lose premium on next gated request."""
    ...
    # Try a premium-gated endpoint (PUT uses require_premium, GET does not)
    r = client.put(
    ...
    # Should be 403 because subscription expired and require_premium recomputes
    assert r.status_code == 403

    # Verify the boolean was corrected
    db.refresh(u)
    assert u.has_premium_access is False
...
def test_lifetime_user_not_affected_by_sub_expiry_check(client, db):
    """A lifetime user should pass require_premium without hitting sub expiry logic."""
    u = _make_user(db, premium=True, lifetime=True)
...
def test_premium_user_hits_device_cap(client, db):
    """Premium users should still be capped at MAX_DEVICES."""
    u = _make_user(db, premium=True, lifetime=True)
    ...
    # 6th should fail with structured error
    r = client.post(
        "/me/devices/register",
        json={
            "device_type": "phone",
    ...
    body = r.json()["detail"]
    assert body["code"] == "device_cap_reached"
