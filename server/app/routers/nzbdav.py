...
All endpoints are `Depends(require_premium)` except GET /status which is
`get_current_user_id` (read-only). API responses are strictly Torve-native;
upstream semantics never leak to clients.
"""
...
async def resolve_endpoint(
    ...
    result = await resolve_service.resolve(
        db, user_id=uid, content_id=body.content_id, candidate=cand
    )
    stream: ResolvedStream | None = None
    if (
...
async def handoff_endpoint(
    ...
    This is a premium-gated flow on the caller side (the handoff token is
    only minted from resolve which requires premium), but the endpoint
